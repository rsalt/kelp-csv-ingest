...
/**
 * Print age distribution to stdout (after ingest).
 * Buckets: <20, 20–40, 41–60, >60 (non-overlapping).
 */
async function printAgeDistribution(databaseUrl) {
  ...
  const { rows } = await pool.query(`
    ...
    counts AS (
      SELECT age_group, COUNT(*)::double precision AS cnt
      FROM bucketed
      GROUP BY age_group
    ),
    total AS (
      SELECT COALESCE(SUM(cnt), 0) AS t FROM counts
    )
    SELECT
      c.age_group,
  ...
  const byGroup = new Map(rows.map((r) => [r.age_group, r.pct]));

  console.log('');
  console.log('Age-Group\t% Distribution');
  for (const label of ORDER) {
    const pct = byGroup.has(label) ? byGroup.get(label) : 0;
